...
$(function () {
    ...
    $(document).keypress(banBackSpace);
    //禁止后退键  作用于IE、Chrome  
...
function getAjaxSetTimeout(url, method, parm, timeout=2000, callBack, callBackError) {
    $.ajax({
        method: method,
        dataType: "json",
        contentType: 'application/json; charset=UTF-8',
...
function getAjaxByHeader(url, parm, token,callBack) {
    $.ajax({
        type: 'post',
        dataType: "text",
        url: url,
...
function getAjaxCookie(url, parm, callBack) {
    $.ajax({
        type: 'post',
        xhrFields:{
...
function getAjaxCookieByAsync(url, parm) {
    var defer = $.Deferred();
    $.ajax({
        type: 'post',
        xhrFields:{
...
function AsyncAjax(url,parm,callBack){
    $.when(AjaxRequst(url,parm)).done(callBack);
}
function getAjax2(url, parm, callBack) {
    $.ajax({
        type: 'post',
        dataType: "json",
        url: url,
...
//extention--------------
function PQLoadGrid4Post(obj_ID, url, colM, sort, PageSize, topVisible) {
    GetRowIndex = -1;
    var dataModel = {
        location: "remote",
        sorting: "remote",
...
function combotree(Objkey, width, height, data) {
    ...
    $("#div_treeshow").css("left", Y).css("top", X + 33).show();
...
function putDate(obj) {//onkeyup
    var v = obj.value;
    if (event.keyCode == 8 || event.keyCode == 46) { return false; }
    if (v.length == 4 || v.length == 7) { obj.value = v + "-"; }
}

function validNUM(obj) {//onkeypress
    var v = obj.value;
    if (event.keyCode < 47 || event.keyCode > 57) { event.returnValue = false; }
    else if (event.keyCode == 47 && v.length != 4 && v.length != 7) { event.returnValue = false; }
    else if (event.keyCode != 47 && (v.length == 4 || v.length == 7)) { obj.value = v + "-"; }
}

//日期验证，符合返回true,否则返回false
function CheckData(obj) {
    var str = obj.value;
    var reg = /^((((1[6-9]|[2-9]\d)\d{2})-(0?[13578]|1[02])-(0?[1-9]|[12]\d|3[01]))|(((1[6-9]|[2-9]\d)\d{2})-(0?[13456789]|1[012])-(0?[1-9]|[12]\d|30))|(((1[6-9]|[2-9]\d)\d{2})-0?2-(0?[1-9]|1\d|2[0-8]))|(((1[6-9]|[2-9]\d)(0[48]|[2468][048]|[13579][26])|((16|[2468][048]|[3579][26])00))-0?2-29-))$/;
    if (str && !reg.test(str)) {
        //alert("请输入正确格式的日期!如07/15/2007");
        showTipsMsg("请输入正确格式的日期!如2014-01-02!", 1300, 3);
        return false;
    }
    return true;
}
...
function setDjangoCookie(name,value) {
    var days = 14;
    var exp = new Date();
    exp.setTime(exp.getTime() + days*24*60*60*1000);
    document.cookie = name + "="+ escape (value) + ";expires=" + exp.toGMTString()+"; path=/";
}
function setDjangoUserInfo(name,value){
    var days = 14;
    var exp = new Date();
    exp.setTime(exp.getTime() + days*24*60*60*1000);
    document.cookie = name + "="+ escape (value) + ";expires=" + exp.toGMTString()+"; path=/";
...
function checkPermission() {
    var url="/tp5/login/checkPermission";
    var params="";
    getAjax(url,params,function(rs){
        ...
        if(json.ok=="1"){
            ...
            location.href="/tp5/public/jkxSystem/login.html";
        }
    });
}